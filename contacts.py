...
def contactData(nid):
    ...
    for key in data:
        ...
        value = drupal.field_value(cursor, nid, 'field_kontaktperson_vorname')
        if value is not None: node['first_name'] = value

        # field_kontaktperson_nachname
        value = drupal.field_value(cursor, nid, 'field_kontaktperson_nachname')
        if value is not None: node['second_name'] = value

        # field_kontaktperson_titel
        value = drupal.field_value(cursor, nid, 'field_kontaktperson_titel')
        if value is not None: node['formal_address'] = value

        # field_kontaktperson_telefon
        value = drupal.field_value(cursor, nid, 'field_kontaktperson_telefon')
        if value is not None: node['phone'] = value

        # field_kontaktperson_email
        value = drupal.field_value(cursor, nid, 'field_kontaktperson_email')
